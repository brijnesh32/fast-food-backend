...
def food_list(request):
    ...
    for f in foods:
        out.append({
            ...
            "calories": f.calories,
            "protein": f.protein,
            "category": {"id": str(f.category.id), "name": f.category.name} if f.category else None,
...
def order_list(request):
    ...
    for o in orders:
        out.append({
            ...
            "total": o.total,
            "status": o.status,
            "items": [{"food_id": it.food_id, "name": it.name, "quantity": it.quantity, "price": it.price, "customizations": it.customizations} for it in o.items]
        })
    return JsonResponse(out, safe=False)
